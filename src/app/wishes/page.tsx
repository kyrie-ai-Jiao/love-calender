  ...
  return (
    <div className="max-w-lg mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        {wishes.length > 0 && (
          <span className="text-xs text-love-400 font-medium">
      ...
      {!hasSetup ? (
        <Card className="text-center py-12">
          <ListTodo className="w-8 h-8 text-love-200 mx-auto mb-3" />
        ...
        <>
          ...
          {wishes.length > 0 && (
            <div className="bg-gray-100 rounded-full h-2 overflow-hidden">
              <div
                className="bg-love-400 h-full rounded-full transition-all duration-500"
          ...
          {wishes.length === 0 ? (
            <Card className="text-center py-12">
              <ListTodo className="w-10 h-10 text-love-200 mx-auto mb-3" />
              <p className="text-sm text-gray-400">还没有愿望</p>
              <p className="text-xs text-gray-300 mt-1">把想一起去的地方、想做的事写下来</p>
            </Card>
          ) : (
  ...
  return (
    <div
      className={`flex items-start gap-3 p-4 rounded-2xl transition-all ${
        wish.completed ? "bg-gray-50 opacity-60" : "bg-white border border-gray-100 shadow-sm"
      }`}
    >
      <button
        onClick={() => onToggle(wish.id)}
        className="mt-0.5 flex-shrink-0 text-love-400 hover:text-love-500 transition-colors cursor-pointer"
      >
        {wish.completed ? (
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`text-sm ${
            wish.completed ? "text-gray-400 line-through" : "text-gray-800 font-medium"
          }`}
        >
          {wish.title}
        </p>
        {wish.description && (
          <p className="text-xs text-gray-400 mt-0.5">{wish.description}</p>
        )}
        <p className="text-xs text-gray-300 mt-1">{wish.createdAt}</p>
      </div>
